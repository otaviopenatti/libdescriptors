...
int FirstPositionNonBlank(char *buffer) {
	int i=0;
	char *c = buffer;	
	while(1) {
		if(*c == '\0') return -1;
...
void ReadNonCommentLine(FILE *fp, char *buffer) {
	int p=0;
	do {
		/*
		it's possible buffer overflow here. 
		There is a workaround: read char by char and stop when buffer is full or end of line is get.
		*/
		fscanf(fp,"%[^\n]\n", buffer);
		p = FirstPositionNonBlank(buffer);
	} while(p == -1 || buffer[p] == '#');
}
